...
int mySliderValue = 50;
char myTextInput[256] = "Default Text";

void gui::SaveConfig() noexcept {
	std::ofstream file("config.cfg");
	if (!file.is_open())
		MessageBox(NULL, "Nastala chyba pøi manipulaci s konfiguraèním souborem", "Chyba", MB_OK | MB_ICONERROR);

	file << "Checkbox=" << myCheckbox << "\n";
	file << "SliderValue=" << mySliderValue << "\n";
	file << "TextInput=" << myTextInput << "\n";
...
void gui::Render() noexcept
{
	...
	if (ImGui::Checkbox("Enable Feature", &myCheckbox)) {
		gui::SaveConfig();
	}
	if (ImGui::SliderInt("Value", &mySliderValue, 0, 100)) {
		gui::SaveConfig();
	}
	if (ImGui::InputText("Text", myTextInput, sizeof(myTextInput))) {
		gui::SaveConfig();
	}
